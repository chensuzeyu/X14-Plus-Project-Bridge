...
新增正式本地图片管线、只读图片发现工具及默认关闭的两个实验入口。默认 15 个协议工具，其中 1 个仅供卡片使用。安装图片能力还需为配置中的 Python 安装 Pillow；当前能力以 14-local-image-access.md 为准，任务验收以 13-task-vision-acceptance.md 为准。早期项目边界、20 MiB/2048 像素限制属于历史版本。

图片代码变更后验证 Python 预处理、卡片协议、错误交接和实际服务资源版本；自动化测试不能替代网页视觉验收。当前 18 项自动测试已通过，实际服务已验证项目外图片发现和读取。网页任务已产出文件，复核修正一处状态字段；完整工具调用顺序未独立核实。


## 2026-09-21 SSH 图片补充

同一 view_project_images 已支持 SSH 项目内相对/绝对路径；远端仅标准库读取，Pillow 处理在本机完成。无需安装远端依赖或更新远端引擎。资源 v5，工具总数不变。19 项自动测试通过；实际服务 RIT1.png 图片字节、尺寸、源文件哈希及交付去重均通过，网页视觉待验收。详见 15-ssh-image-access.md。

修复过滤任务环境导致 ProgramData 丢失、Windows OpenSSH 静默退出 255 的问题。任务保留该变量，Manage-Bridge 启动时可从系统注册表恢复；重载时只终止 PID 与启动时间匹配的 Bridge 进程，不停止 Tunnel/代理。

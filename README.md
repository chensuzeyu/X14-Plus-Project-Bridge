...
- 默认 15 个 MCP 工具：12 个项目工具、1 个本地图片查找工具、正式 view_project_images，以及 1 个仅供卡片使用的提交去重工具（模型面向 14 个）。2 个实验工具默认关闭，可通过 enable_vision_experiments: true 启用。本地截图网页识读已有成功反馈；任务内看图后继续编辑已有产物验证（发现并修正一处状态字段误解），见 docs/13-task-vision-acceptance.md。
...
本地图片可跨项目读取：绝对路径不需要 project_id；discover_local_images 可从磁盘/目录逐步查找图片。详见 docs/14-local-image-access.md。
